...
DEFAULT_REPO_ARCHIVE_URL="https://github.com/tbladh/armarius-skill/archive/refs/heads/main.tar.gz"
...
Install Armarius globally for Codex, Claude, Cursor, Kiro, and Cline.
GitHub Copilot and Windsurf piggyback on ~/.agents/skills by default.

Options:
  --all               Install to the default broad harness set; excludes native Copilot/Windsurf paths
  ...
  --copilot           Install only to GitHub Copilot's native path
  --windsurf          Install only to Windsurf's native path
...
Environment:
  ARMARIUS_REPO_ARCHIVE_URL  Override the GitHub archive used by raw-script bootstrap.
  ARMARIUS_INSTALL_HOME      Override the user home used for installation targets.
EOF
}
...
resolve_repo_root() {
  ...
  local archive_url="${ARMARIUS_REPO_ARCHIVE_URL:-${DEFAULT_REPO_ARCHIVE_URL}}"
  ARCHIVE_TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/armarius-install.XXXXXX")"
  ...
  if [[ -z "${extracted_root}" || ! -f "${extracted_root}/config/defaults.env" ]]; then
    echo "Could not find an Armarius repository in ${archive_url}." >&2
    return 1
  fi
...
if [[ "${TARGET_MODE}" == "all" ]]; then
  # Keep aligned with Engram: Copilot and Windsurf use ~/.agents/skills by default.
  # Their native paths are explicit opt-ins through --copilot and --windsurf.
