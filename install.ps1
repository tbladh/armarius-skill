...
$DefaultRepoArchiveUrl = "https://github.com/tbladh/armarius-skill/archive/refs/heads/main.zip"
$ArchiveTempDir = $null
$RenderRoot = $null
...
function Resolve-RepoRoot {
    ...
    $archiveUrl = if ($env:ARMARIUS_REPO_ARCHIVE_URL) { $env:ARMARIUS_REPO_ARCHIVE_URL } else { $DefaultRepoArchiveUrl }
    $script:ArchiveTempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("armarius-install-" + [guid]::NewGuid())
    ...
    if (!$root) {
        throw "Could not find an Armarius repository in $archiveUrl."
    }
...
try {
    if ([string]::IsNullOrWhiteSpace($InstallHome)) {
        $InstallHome = if ($env:ARMARIUS_INSTALL_HOME) { $env:ARMARIUS_INSTALL_HOME } else { [Environment]::GetFolderPath("UserProfile") }
    }

    $repoRoot = Resolve-RepoRoot
    $config = Read-Defaults -Path (Join-Path $repoRoot "config/defaults.env")
    $RenderRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("armarius-render-" + [guid]::NewGuid())
