...
def main() -> int:
    parser = argparse.ArgumentParser(description="Render the Armarius skill template.")
    ...
    for path in rendered_root.rglob("*"):
        ...
        unresolved = [key for key in replacements if key in rendered_text]
        if unresolved:
            raise ValueError(f"Unresolved placeholder found in {path}: {', '.join(unresolved)}")
